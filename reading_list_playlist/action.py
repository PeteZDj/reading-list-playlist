...
import calibre_plugins.reading_list_playlist.config as cfg
from calibre_plugins.reading_list_playlist.common_icons import set_plugin_icon_resources, get_icon
from calibre_plugins.reading_list_playlist.common_menus import unregister_menu_actions, create_menu_action_unique
from calibre_plugins.reading_list_playlist.dialogs import EditListDialog, MoveBooksDialog
...
class ReadingListAction(InterfaceAction):
    ...
    name = 'Reading List Playlist'
    # Create our top-level menu/toolbar action (text, icon_path, tooltip, keyboard shortcut)
    action_spec = (_('Reading List Playlist'), None, _('View or edit playlist-style lists of books'), None)
    ...
    def rebuild_menus(self):
        with self.menus_lock:
           ...
           # The list view menu items by default appear in a View list submenu
            ...
            m.addSeparator()
            create_menu_action_unique(self, m, _('Keep Genre view flat'), 'chapters.png',
    ...
    def _genre_category_key(self):
        try:
            ...
            preferred = '#genre'
            if preferred in field_metadata and field_metadata[preferred].get('is_category', False):
                return preferred
            for key, metadata in field_metadata.items():
                ...
                if key == preferred or label.lower() == 'genre' or name.lower() == 'genre':
                    return key
        except Exception as e:
            if DEBUG:
                prints('Reading List Playlist: unable to inspect Genre category: {}'.format(e))
        return None

    def _keep_genre_view_flat(self, show_dialog=True):
        category_key = self._genre_category_key()
        if not category_key:
            if show_dialog:
                return info_dialog(self.gui, _('Genre category not found'),
                                   _('No Tag Browser category named Genre was found in this library.'),
        ...
        prefs_key = 'tag_browser_dont_collapse'
        db = self.gui.current_db
        current = list(db.prefs.get(prefs_key, []) or [])
        changed = category_key not in current
        if changed:
            current.append(category_key)
            db.prefs.set(prefs_key, current)
            try:
                self.gui.tags_view.recount()
            except Exception:
                pass
        ...
        if show_dialog:
            if changed:
                message = _('Genre will stay in the normal flat Tag Browser list instead of switching to grouped ranges.')
            else:
                message = _('Genre is already configured to stay in the normal flat Tag Browser list.')
            return info_dialog(self.gui, _('Genre view kept flat'), message, show=True)
        return changed

    def _keep_genre_view_flat_from_menu(self, *args):
        return self._keep_genre_view_flat(show_dialog=True)
