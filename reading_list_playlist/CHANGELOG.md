...
- Also register Reading List Playlist in the cover browser right-click context
  menu, which is used by Calibre's cover grid view.
...
- Renamed the plugin package to `reading_list_playlist`.
- Added automatic Genre Tag Browser flat-view handling via calibre's
  `tag_browser_dont_collapse` preference.
- Added a `Keep Genre view flat` menu action.
- Added project README/NOTICE documentation and source packaging script.
